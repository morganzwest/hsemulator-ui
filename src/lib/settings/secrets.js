...
async function apiRequest(path, { method, body }) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      ...
      errorMessage =
        CICD_ERROR_MESSAGES[CICD_ERROR_TYPES.INVALID_SECRET].message;
      guidance = CICD_ERROR_MESSAGES[CICD_ERROR_TYPES.INVALID_SECRET].guidance;
      recoverable =
        CICD_ERROR_MESSAGES[CICD_ERROR_TYPES.INVALID_SECRET].recoverable;
    } else if (res.status === 403) {
      errorType = CICD_ERROR_TYPES.MISSING_SCOPE;
      errorMessage =
        CICD_ERROR_MESSAGES[CICD_ERROR_TYPES.MISSING_SCOPE].message;
      guidance = CICD_ERROR_MESSAGES[CICD_ERROR_TYPES.MISSING_SCOPE].guidance;
      recoverable =
        CICD_ERROR_MESSAGES[CICD_ERROR_TYPES.MISSING_SCOPE].recoverable;
    }
...
export async function fetchPortalSecrets(portalId) {
  ...
  if (secretsError) {
    console.error(
      "[secrets][fetchPortalSecrets] Supabase error:",
      secretsError,
    );
...
export async function deleteSecret(secretId, ctx) {
  ...
  await apiRequest(`/api/runtime/secrets/${secretId}`, {
    method: "DELETE",
    body: {
